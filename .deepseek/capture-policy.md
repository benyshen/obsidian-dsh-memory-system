...
| 字段 | 对象 | 默认 | 说明 |
|---|---|---|---|
| `idea` | 💡 可捕捉的想法（写入 inbox 备忘录） | `ask` | 识别到一般性思路/方法/技巧/观点时先问再写（既有行为） |
| `fact` | 新事实 / 事件 / 指令（写入 records 原子卡） | `auto` | 三写协议第 2 步的默认档位 |
| `preference` | 稳定偏好 / 授权（写入 profile 与 records） | `auto` | 三写协议第 3 步的默认档位 |
