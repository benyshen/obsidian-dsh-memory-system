/**
 * aimemory-notes — dedicated AImemory note tools for the `aimemory` dsh agent
 * preset:
 ...
 * same pattern as the dsh-obsidian-assistant reference plugin (the upstream
 ...
 * under .deepseek/cache/ (the same convention as aimemory-memory.mjs).
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, isAbsolute, join, resolve } from "node:path";

export const name = "aimemory-notes";
export const inject = ["tools", "fs", "systemPrompt", "loader"];

/**
 * Load `defineTool` without a static bare import. A local agent-preset plugin
 * lives under `$DSH_HOME/.agent-presets/aimemory/`, where Node's ordinary ESM
...
async function loadDefineTool(ctx) {
  ...
  if (typeof internal?.import === "function") {
    ...
    if (base === undefined) {
      throw new Error("aimemory-notes: cannot resolve the harness module base (ctx.root.baseUrl is unset)");
    }
    const module = await internal.import("@deepseek-ai/dsh-tools", base, {});
    if (typeof module.defineTool !== "function") {
      throw new Error("aimemory-notes: @deepseek-ai/dsh-tools did not export defineTool");
    }
...
function positiveInteger(value, fallback, label) {
  ...
  if (!Number.isInteger(number) || number < 1) {
    throw new Error(`aimemory-notes: ${label} must be a positive integer`);
  }
  return number;
}
...
/** Resolve the vault the tools act on: preset config, DSH_AIMEMORY_VAULT, session cwd. */
function vaultRootFor(exec, config) {
  const fromConfig = config.vaultRoot;
  const fromEnv = (process.env.DSH_AIMEMORY_VAULT ?? "").trim();
  ...
  if (raw === "") {
    throw new Error("aimemory-notes: cannot determine the vault directory; set DSH_AIMEMORY_VAULT or configure vaultRoot in the preset.");
  }
  if (!isAbsolute(raw)) {
    throw new Error(`aimemory-notes: vaultRoot must be an absolute path, got ${JSON.stringify(raw)}`);
  }
  return resolve(raw);
}
...
async function resolveVault(ctx, exec, config) {
  ...
  if (info === undefined) throw new Error(`aimemory-notes: vault root not found: ${rootPath}`);
  if (info.type !== "directory") throw new Error(`aimemory-notes: vault root is not a directory: ${rootPath}`);
  return { rootPath, rootTarget };
}

function requireInsideVault(ctx, rootTarget, target) {
  if (!ctx.fs.contains(rootTarget, target)) {
    throw new Error(`aimemory-notes: path escapes the vault: ${target.displayPath}`);
  }
}
 ...
 * aimemory-memory.mjs writes uses/last_used back in block style).
 *
 *   hook:
...
function normalizeNoteRelPath(input) {
  ...
  if (rawInput.includes("\u0000")) throw new Error("aimemory-notes: note path contains a NUL byte");
  if (isAbsolute(rawInput) || /^[A-Za-z]:[\\/]/.test(rawInput)) {
    throw new Error(`aimemory-notes: note path must be vault-relative, got ${JSON.stringify(rawInput)}`);
  }
  let rel = rawInput.replace(/\\/g, "/").replace(/^\.\/+/, "");
  if (rel.startsWith("/")) rel = rel.replace(/^\/+/, "");
  if (rel === "" || rel === "." || rel.endsWith("/")) {
    throw new Error(`aimemory-notes: invalid note path ${JSON.stringify(rawInput)}`);
  }
  const parts = rel.split("/");
  if (parts.some((part) => part === "" || part === "." || part === ".." || part.includes(":"))) {
    throw new Error(`aimemory-notes: note path escapes the vault or is invalid: ${JSON.stringify(rawInput)}`);
  }
 ...
 * aimemory-memory.mjs merges this into the cards' hook.uses / last_used and
...
export async function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    name: "tool:aimemory-notes",
    order: 103,
    text:
      "Use the dedicated AImemory note tools for note-level operations: " +
  ...
  ctx.tools.register(defineTool({
    name: "note_create",
    description: `Create a new markdown note in the AImemory 记忆库 at the given vault-relative path. The full markdown (including any frontmatter) is written exactly as provided. This tool refuses to overwrite an existing note and has no overwrite flag; to change an existing note, use read + edit/write after the user asks for it.`,
    parameters: {
  ...
  ctx.tools.register(defineTool({
    name: "note_links",
    description: `Query wikilink backlinks in the AImemory 记忆库: which notes link TO a note. Pass a note name (with or without the ".md" extension) to see only its incoming links; omit the argument to return the full note-name -> linking-notes map.`,
    parameters: {
