/**
 * aimemory-memory — cross-session memory injection for the `aimemory` dsh
 * agent preset. It also applies the sibling aimemory-notes.mjs plugin on the
 ...
 *   4. Appends one bounded "aimemory:memory" section to the assembled system
 *      prompt.
 *
...
export const name = "aimemory-memory";
...
export function scanZstdFrames(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (buffer.readUInt32LE(offset) !== ZSTD_MAGIC) {
      throw new Error(`aimemory-memory: bad zstd frame magic at byte ${offset}`);
    }
    offset += 4;
    if (offset === buffer.length) return frames; // torn header after magic
    const descriptor = buffer.readUInt8(offset);
    offset += 1;
    if ((descriptor & 24) !== 0) throw new Error(`aimemory-memory: reserved zstd frame-header bit at byte ${offset - 1}`);
    ...
    for (;;) {
      ...
      if (blockType === 3) throw new Error(`aimemory-memory: reserved zstd block type at byte ${offset - 3}`);
...
export function buildMemorySection({ vaultRoot, sessionsRoot, maxHistoryEntries, maxHistoryChars, cacheTtlMs }, currentSessionId, dialogueIndex, auditReport, memoText) {
  ...
  const lines = [
    "## 分层长期记忆（由 aimemory-memory 自动注入；导航层在此，证据层在磁盘）",
    "",
  ...
  // When the AImemory plugin provides its loopback link server, tell the
  // agent to render note references in replies as clickable links so the
  // user can jump straight into AImemory from the sidebar iframe. The same
  // server's /open and /feedback endpoints are guarded by a CSRF token that
  // the plugin passes via DSH_AIMEMORY_FEEDBACK_TOKEN — the rendered link
  // templates MUST carry it as t= or the click is rejected with 403.
  const linkBaseUrl = process.env.DSH_AIMEMORY_LINK_URL?.trim() ?? "";
  if (linkBaseUrl !== "") {
    const linkToken = process.env.DSH_AIMEMORY_FEEDBACK_TOKEN?.trim() ?? "";
    const tokenSuffix = linkToken === "" ? "" : `&t=${linkToken}`;
    lines.push(
      `- 回复正文中引用笔记时，使用可点击链接：[标题](${linkBaseUrl}/open?path=<vault 相对路径，需 URL 编码>${tokenSuffix})；`,
      "  点击即可在 AImemory 中打开对应笔记。笔记文件内部仍写 [[wikilink]]，两者不要混用。",
      `- 引用记忆卡时标注验证等级徽标：✅用户确认（hook.verified=user-confirmed）/ ⚖️互证（cross-referenced）/ ❓单源（single-source 或缺失）。`,
      `- 本回复依据了记忆卡时，在末尾给反馈链接（path 为该卡 vault 相对路径，需 URL 编码）：[✅ 这条对](${linkBaseUrl}/feedback?path=<卡路径>&action=confirm${tokenSuffix}) [❌ 这条错](${linkBaseUrl}/feedback?path=<卡路径>&action=wrong${tokenSuffix})；用户点击后由 AImemory 插件直接改写验证等级与成功率，无需你代劳。`
    );
  }
  ...
  if (auditReport?.report !== undefined && auditReport.report !== "") {
    lines.push("", "### 记忆体检（aimemory-memory 确定性扫描，见 .deepseek/cache/memory-audit.json）", "", auditReport.report,
...
function normalizeConfig(config) {
  const vaultRaw = (process.env.DSH_AIMEMORY_VAULT ?? config.vaultRoot ?? "").trim();
  ...
  if (!isAbsolute(sessionsRoot)) throw new TypeError("aimemory-memory: sessionsRoot must be an absolute path");
...
class MemoryEngine {
  ...
  async sectionForAgent(agent) {
    ...
    try {
      ...
      if (vaultRoot === "") {
        return "## 长期记忆（aimemory-memory 未配置）\n\n" +
          "当前会话没有可用的 vault 工作目录；设置 DSH_AIMEMORY_VAULT 或在 preset 配置 vaultRoot。";
      }
      ...
      return `## 长期记忆（aimemory-memory 暂不可用）\n\n${String(error)}`;
    }
  }
}
...
export async function apply(ctx, config) {
  ...
  const notes = await import("./aimemory-notes.mjs");
  ...
  ctx.on("system-prompt/assemble", async (assembly, context, next) => {
    ...
    return {
      ...assembled,
      sections: [...assembled.sections, { name: "aimemory:memory", text }]
    };
  });
